...
def calc_proprty(s1,s2,t):
    dist = abs(s1-s2)
    mid = (s1-s2)/2
    s1_jumped = s1>5
    s2_jumped = s2>5
    all_gone = (s1>4.5 and s2>4.5 and t>3.5) or (s1<-4.5 and s2<-4.5 and t<-3.5)
    return dist, mid,s1_jumped,s2_jumped,all_gone
#
#
def arenan(*args):
    for i in args:
...
#
#this is mostly customized for Iran Stock but it can be used for others as well by just some tuning
def calc_edge(ser1,ser2,type,totindex=None,tresh = 1.2,tresh2 =2 ):
    # assert len(ser1.index) == len(ser2.index) == len(totindex),"should have the same len"
    s1, s2, s = get_most(ser1,ser2)
    if type == 1:
        semi_cor = 0
        for i , j ,k in zip(s1,s2,totindex):
            dist ,mid,s1_jumped,s2_jumpled,all_gone = calc_proprty(i,j,k)
            if all_gone:
                continue
            if dist < tresh:
                semi_cor+=1
        ...
        # needs C library
        return  dtw.distance_fast(s1.to_numpy(),s2.to_numpy())
    elif type == 3:
        ...
        days = 0
        tot_dist = 0
        for i , j ,k in zip(ser1,ser2,totindex):
            if not arenan(i,j,k):
                dist ,mid,s1_jumped,s2_jumpled,all_gone = calc_proprty(i,j,k)
                if all_gone:
                    continue
